export class DsrFinanceController {
  ...
  cashUpdate = async (req, res, next) => {
    try {
      const record = await this.dsrFinanceService.saveRecord('cash', { ...req.body, id: req.params.id }, req.currentUser);
      res.json({ record });
    } catch (error) {
      next(error);
    }
  ...
  advanceUpdate = async (req, res, next) => {
    try {
      const record = await this.dsrFinanceService.saveRecord('advance', { ...req.body, id: req.params.id }, req.currentUser);
      res.json({ record });
    } catch (error) {
      next(error);
    }
