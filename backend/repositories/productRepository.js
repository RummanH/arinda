...
export async function listAllActiveProductsLite(client, tenantId) {
  const result = await client.query(
    'SELECT id, name, category, pieces_per_case, purchase_price, selling_price, stock_pieces, order_index FROM products WHERE tenant_id = $1 ORDER BY order_index ASC, name ASC',
    [tenantId],
  );
...
export function insertProduct(client, product) {
  return client.query(
    `INSERT INTO products (id, tenant_id, name, category, pieces_per_case, purchase_price, selling_price, stock_pieces, order_index)
     ...
     RETURNING *`,
    [product.id, product.tenantId, product.name, product.category, product.piecesPerCase, product.purchasePrice, product.sellingPrice, product.stockPieces, product.orderIndex ?? 9999],
  );
}
...
export function updateProduct(client, product) {
  return client.query(
    `UPDATE products
     ...
     RETURNING *`,
    [product.id, product.tenantId, product.name, product.category, product.piecesPerCase, product.purchasePrice, product.sellingPrice, product.orderIndex ?? 9999],
  );
}
