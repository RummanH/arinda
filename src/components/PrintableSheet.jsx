...
export default function PrintableSheet({ sheet, printTarget = false, targetId }) {
  ...
  const totalReturned = rows.reduce((sum, item) => sum + Number(item.returnedPieces || 0), 0);
    ...
    <div id={targetId} className={cx('mx-auto w-full max-w-[210mm] rounded-xl border border-slate-200 bg-white p-6 shadow-soft', printTarget && 'print-target')}>
      <div className="border-b-2 border-slate-950 pb-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-2xl font-black tracking-normal text-slate-950">ARINDA ENTERPRISE</h2>
        ...
        <table className="w-full border-collapse text-left text-xs">
          <thead>
            <tr className="bg-slate-950 text-white">
              ...
              <th className="border border-slate-800 px-2 py-2 text-right">Payable</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((item, index) => (
              <tr key={`${item.productId}-${item.rate}-${index}`} className="print-break-inside-avoid">
                <td className="border border-slate-300 px-2 py-2 font-semibold">{index + 1}</td>
                <td className="border border-slate-300 px-2 py-2 font-semibold text-slate-950">{item.productName}</td>
